...
RUNTIME_ALGORITHM_ENV="${RUNTIME_DIR}/runtime_algorithm.env"
...
# v2.2: 设置界面生成的运行时算法覆盖。
# 这里按模型 task 选择对应阈值，优先级高于模型同名 yaml。
NPU_CORE="auto"
WARMUP_RUNS="3"
if [[ -f "${RUNTIME_ALGORITHM_ENV}" ]]; then
  set -a
  # shellcheck disable=SC1090
  . "${RUNTIME_ALGORITHM_ENV}"
  set +a
  NPU_CORE="${VISIONOPS_RUNTIME_NPU_CORE:-${NPU_CORE}}"
  WARMUP_RUNS="${VISIONOPS_RUNTIME_WARMUP_RUNS:-${WARMUP_RUNS}}"
  case "${TASK}" in
    classification)
      TOPK="${VISIONOPS_RUNTIME_CLASSIFICATION_TOPK:-${TOPK}}"
      ;;
    detection)
      CONF_THRESHOLD="${VISIONOPS_RUNTIME_DETECTION_CONF_THRESHOLD:-${CONF_THRESHOLD}}"
      NMS_THRESHOLD="${VISIONOPS_RUNTIME_DETECTION_NMS_THRESHOLD:-${NMS_THRESHOLD}}"
      ;;
    obb_detection)
      CONF_THRESHOLD="${VISIONOPS_RUNTIME_OBB_CONF_THRESHOLD:-${CONF_THRESHOLD}}"
      NMS_THRESHOLD="${VISIONOPS_RUNTIME_OBB_NMS_THRESHOLD:-${NMS_THRESHOLD}}"
      ;;
    segmentation)
      CONF_THRESHOLD="${VISIONOPS_RUNTIME_SEGMENTATION_CONF_THRESHOLD:-${CONF_THRESHOLD}}"
      NMS_THRESHOLD="${VISIONOPS_RUNTIME_SEGMENTATION_NMS_THRESHOLD:-${NMS_THRESHOLD}}"
      MASK_THRESHOLD="${VISIONOPS_RUNTIME_SEGMENTATION_MASK_THRESHOLD:-${MASK_THRESHOLD}}"
      ;;
  esac
fi
