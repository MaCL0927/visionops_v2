...
def _switch_model_via_system_service(model_path: Path, meta_path: str, meta: Dict[str, Any]) -> Dict[str, Any]:
    """切换到普通单 RKNN engine.py 服务。

    不能只调用旧版 switch_model.sh 改 .env，因为 ROI 双模型部署后
    /etc/systemd/system/visionops-inference.service 可能仍指向 pipeline_engine.py。
    所以这里明确重写 systemd service，把 ExecStart 切回 engine.py。
    """
    engine_path = _resolve_engine_path()
    if not engine_path.exists():
        raise FileNotFoundError(f"未找到普通推理入口 engine.py: {engine_path}")

    task = str(meta.get("task") or "").strip().lower()
    if task not in {"classification", "detection", "obb_detection", "segmentation"}:
        raise ValueError(f"普通单模型 task 无效: {task}")

    try:
        write_runtime_algorithm_env()
    except Exception as exc:
        logger.warning("写 runtime_algorithm.env 失败，继续使用 meta/default 参数: %s", exc)

    port = _validation_port()
    metrics_port = int(os.getenv("METRICS_PORT", "9091"))
    npu_core = str(VALIDATION_NPU_CORE or "auto")
    input_size = meta.get("input_size") or ([224, 224] if task == "classification" else [640, 640])
    try:
        input_size_text = f"{int(input_size[0])},{int(input_size[1])}"
    except Exception:
        input_size_text = "224,224" if task == "classification" else "640,640"

    num_classes = int(meta.get("num_classes") or 1)
    topk = int(meta.get("topk") or min(max(num_classes, 1), int(VALIDATION_TOPK)))
    conf_threshold = float(meta.get("conf_threshold") if meta.get("conf_threshold") is not None else 0.25)
    nms_threshold = float(meta.get("nms_threshold") if meta.get("nms_threshold") is not None else 0.45)
    mask_threshold = float(meta.get("mask_threshold") if meta.get("mask_threshold") is not None else 0.5)

    # 明确清理旧服务与旧进程。尤其是从 roi_classification 切回单模型时，
    # 必须停止仍指向 pipeline_engine.py 的 systemd 服务。
    _stop_validation_process()
    subprocess.run(
        ["sudo", "-n", "systemctl", "stop", "visionops-inference"],
    ...
    if not _wait_validation_port_free(timeout=5.0):
        raise RuntimeError(f"验证端口 {port} 仍被占用，无法启动单模型推理服务")

    env_content = f"""# Auto generated by collector validation_infer.py for single RKNN model
TASK={task}
VISIONOPS_TASK={task}
...
Description=VisionOps RK3588 Inference Service
...
ExecStart=/opt/visionops/venv/bin/python {engine_path} --model ${{MODEL_PATH}} --task ${{TASK}} --host 0.0.0.0 --port ${{PORT}} --npu-core ${{NPU_CORE}} --num-classes ${{NUM_CLASSES}} --class-names-file ${{CLASS_NAMES_FILE}} --metrics-port ${{METRICS_PORT}} --conf-threshold ${{CONF_THRESHOLD}} --nms-threshold ${{NMS_THRESHOLD}} --mask-threshold ${{MASK_THRESHOLD}} --topk ${{TOPK}} --warmup-runs ${{WARMUP_RUNS}}
Restart=always
RestartSec=5
TimeoutStartSec=30
TimeoutStopSec=10
...
"""
    ...
    while time.time() < deadline:
        try:
            ...
            if _health_matches_model(health, model_path, meta_path, meta, ignore_force=True):
                return {"health": health, "stdout": "systemd single-model engine.py service restarted"}
    ...
    raise RuntimeError(f"单模型服务启动后健康检查失败: {last_err}\n最近日志:\n{logs}")
...
def infer_image_with_model(model_name: str, image_path: Path) -> Dict[str, Any]:
    ...
    if task == "classification":
        ...
        root_roi = raw.get("roi") if isinstance(raw.get("roi"), dict) else {}
        # pipeline_engine.py 会在每个 prediction 里放入 roi；
        # 这里仍做一次兜底透传，避免旧版返回只有根级 roi 时前端拿不到 final ROI。
        if root_roi and predictions:
            predictions[0].setdefault("roi", root_roi)
        base["predictions"] = predictions
        base["detection"] = _summarize_detection({"predictions": predictions})
        base["roi"] = root_roi or raw.get("roi")
        ...
        roi_for_result = predictions[0].get("roi") if predictions and isinstance(predictions[0].get("roi"), dict) else root_roi
        base["result"] = {
            ...
            "roi_mode": roi_for_result.get("mode") if isinstance(roi_for_result, dict) else None,
            "roi_pipeline_mode": roi_for_result.get("pipeline_mode") if isinstance(roi_for_result, dict) else None,
        }
