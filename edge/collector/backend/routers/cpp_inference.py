...
from backend.services.hp60c_sdk_bridge_client import (
    Hp60cSdkBridgeError,
    get_binary as hp60c_sdk_bridge_get_binary,
    get_json as hp60c_sdk_bridge_get_json,
    post_json as hp60c_sdk_bridge_post_json,
)
...
from backend.services.cpp_runtime_settings import (
    ...
    restart_cpp_service,
    restart_hp60c_sdk_bridge_service,
...
def _hp60c_sdk_json_or_502(func, *args, **kwargs) -> Dict[str, Any]:
    """Proxy a JSON request to the independent HP60C Angstrong SDK C++ bridge."""
    _ensure_enabled()
    first_error = None
    try:
        return func(*args, **kwargs)
    except Hp60cSdkBridgeError as exc:
        first_error = exc

    restart_info = restart_hp60c_sdk_bridge_service()
    try:
        return func(*args, **kwargs)
    except Hp60cSdkBridgeError as exc:
        detail = {
            ...
            "hint": "请检查 sudo systemctl status visionops-hp60c-sdk-bridge.service 和 journalctl -u visionops-hp60c-sdk-bridge.service -n 80",
        }
...
# ---------------------------------------------------------------------------
# HP60C Angstrong SDK C++ bridge proxy
# ---------------------------------------------------------------------------
# These endpoints replace the older ROS1 topic bridge. The SDK bridge owns the
# camera directly through Angstrong C++ SDK and exposes the same HTTP snapshot
# and MJPEG endpoints used by VisionOps.

@router.get("/hp60c_sdk/health")
def cpp_hp60c_sdk_bridge_health() -> Dict[str, Any]:
    return _hp60c_sdk_json_or_502(hp60c_sdk_bridge_get_json, "/health")


@router.post("/hp60c_sdk/stream/start")
def cpp_hp60c_sdk_stream_start(request: Request) -> Dict[str, Any]:
    return _hp60c_sdk_json_or_502(hp60c_sdk_bridge_post_json, "/stream/start", query_string=request.scope.get("query_string"))


@router.post("/hp60c_sdk/stream/stop")
def cpp_hp60c_sdk_stream_stop(request: Request) -> Dict[str, Any]:
    return _hp60c_sdk_json_or_502(hp60c_sdk_bridge_post_json, "/stream/stop", query_string=request.scope.get("query_string"))


@router.get("/hp60c_sdk/stream/status")
def cpp_hp60c_sdk_stream_status(request: Request) -> Dict[str, Any]:
    return _hp60c_sdk_json_or_502(hp60c_sdk_bridge_get_json, "/stream/status", query_string=request.scope.get("query_string"))


@router.get("/hp60c_sdk/stream/snapshot.jpg")
def cpp_hp60c_sdk_stream_snapshot(request: Request) -> Response:
    _ensure_enabled()
    first_error = None
    try:
        data, content_type = hp60c_sdk_bridge_get_binary("/stream/snapshot.jpg")
    except Hp60cSdkBridgeError as exc:
        first_error = exc
        restart_info = restart_hp60c_sdk_bridge_service()
        try:
            data, content_type = hp60c_sdk_bridge_get_binary("/stream/snapshot.jpg")
        except Hp60cSdkBridgeError as exc2:
            detail = {
                "error": str(exc2),
                "first_error": str(first_error),
                "restart": restart_info,
