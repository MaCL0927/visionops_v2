...
def read_hr(client, address: int, count: int, unit_id: int):
    ...
    for fn in attempts:
        try:
            resp = fn()
            # Some pymodbus versions accept the keyword but send the wrong Unit ID.
            # If server ignores that Unit ID, try the next keyword form before giving up.
...
def main() -> int:
    ...
    try:
        for addr, value, name in [
            (base + 0, 0, "trigger_cmd=0"),
            (base + 1, seq, "trigger_seq"),
            (base + 0, 1, "trigger_cmd=1"),
        ]:
            wr = write_reg(client, addr, value, args.unit_id)
            if hasattr(wr, "isError") and wr.isError():
                print(f"[ERROR] write {name} failed: {wr}")
                return 3
            if name == "trigger_cmd=0":
                time.sleep(0.1)
