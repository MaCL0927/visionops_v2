...
# -*- coding: utf-8 -*-
"""
VisionOps Carton Partition Check Modbus TCP Service

Standalone partition service using the robot protocol register map:
  0   Vision heartbeat
  1   partition placement result: 0=no trigger/no result, 1=OK, 2=NG/ERROR
  ...
  101 partition check trigger: 0=idle, 1=trigger
  103 coordinate recognition trigger: 0=idle, 1=trigger

Coordinates are not cleared when trigger 103 returns to 0.
"""
...
SINGLE_SLAVE = core.getenv_int("VISIONOPS_PARTITION_MODBUS_SINGLE_SLAVE", 1)
ADDRESS_BASE = core.getenv_int("VISIONOPS_PARTITION_ADDRESS_BASE", 0)
REGISTER_COUNT = core.getenv_int("VISIONOPS_PARTITION_REGISTER_COUNT", 200)
...
def safe_coord(v: Any) -> Optional[int]:
    try:
...
def write_coordinates(context: ModbusServerContext, result: Dict[str, Any]) -> None:
    try:
    ...
    for cell in cells:
        ...
        try:
            ...
            continue
        if 0 <= sid < 40:
            x = safe_coord(cell.get("cx"))
            y = safe_coord(cell.get("cy"))
            if x is not None:
                coords[2 * sid] = x
            if y is not None:
                coords[2 * sid + 1] = y
...
def save_debug(task_name: str, rgb_bytes: bytes, payload: Dict[str, Any], result: Dict[str, Any]) -> None:
    if SAVE_EVERY_TRIGGER != 1:
        return
    try:
        out_dir = Path(SAVE_RESULT_DIR) / task_name if task_name else Path(SAVE_RESULT_DIR)
...
class PartitionProtocolService:
    def __init__(self, context: ModbusServerContext) -> None:
        self.context = context
        self.lock = threading.Lock()
        self.busy = False
        self.heartbeat = 0
        self.last_partition_cmd = 0
        self.last_coord_cmd = 0
    ...
    def poll_loop(self) -> None:
        logging.info("Carton partition robot-protocol trigger poll started")
        while True:
            try:
                ...
                if not self.busy:
                    if part_cmd == 1 and self.last_partition_cmd == 0:
                        self.last_partition_cmd = 1
                        threading.Thread(target=self.handle_task, args=(REG_PARTITION_RESULT, False, "partition_check"), daemon=True).start()
                    elif coord_cmd == 1 and self.last_coord_cmd == 0:
                        self.last_coord_cmd = 1
                        threading.Thread(target=self.handle_task, args=(REG_COORD_RESULT, True, "coordinate_check"), daemon=True).start()
            except Exception:
                logging.exception("trigger poll error")
            time.sleep(POLL_INTERVAL_S)

    def handle_task(self, result_reg: int, write_coord: bool, task_name: str) -> None:
        with self.lock:
            if self.busy:
                return
            self.busy = True
        ...
        try:
            ...
            if write_coord:
                write_coordinates(self.context, result)
            code = result_code(result)
            set_reg(self.context, result_reg, code)
            rgb_bytes = result.pop("_rgb_bytes", b"")
            payload = result.pop("_infer_payload", {})
            save_debug(task_name, rgb_bytes, payload, result)
            logging.info("%s done: final=%s code=%d reason=%s time=%dms", task_name, result.get("final_result"), code, result.get("reason"), int((time.time()-start)*1000))
        except Exception as exc:
            logging.error("%s failed: %s", task_name, exc)
            logging.debug("%s", traceback.format_exc())
            set_reg(self.context, result_reg, RESULT_NG)
        finally:
            with self.lock:
                self.busy = False
...
def main() -> int:
    ...
    service = PartitionProtocolService(context)
    ...
    if ModbusDeviceIdentification is not None:
        ...
        identity.ProductCode = "PARTITION_PROTO"
        identity.ProductName = "VisionOps Carton Partition Robot Protocol"
        identity.ModelName = "LB3576 HP60C Carton Partition Check"
        identity.MajorMinorRevision = "1.0"

    logging.info("starting carton partition protocol service: host=%s port=%d unit_id=%d single_slave=%d address_base=%d register_count=%d", HOST, PORT, UNIT_ID, SINGLE_SLAVE, ADDRESS_BASE, max(REGISTER_COUNT, 200))
    logging.info("registers: result 1/3, coords 20~99, triggers 101/103")
