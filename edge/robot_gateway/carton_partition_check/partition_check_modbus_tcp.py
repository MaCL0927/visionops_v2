...
# Modbus-TCP现场上位机/不同pymodbus版本的 Unit ID 可能不一致。
# single_slave=1 时服务端忽略请求中的 unit id，兼容性更好；寄存器访问仍使用 UNIT_ID。
...
def create_context() -> ModbusServerContext:
    ...
    if SINGLE_SLAVE == 1:
        # single=True: accept any Modbus-TCP unit id. This avoids no-response when PLC/client uses 0/1/255 differently.
...
def main() -> int:
    ...
    logging.info(
        "starting carton partition check: host=%s port=%d unit_id=%d single_slave=%d address_base=%d rows=%d cols=%d expected=%d template=%s",
        HOST,
        PORT,
        UNIT_ID,
        SINGLE_SLAVE,
