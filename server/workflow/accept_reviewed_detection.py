...
def normalize_task_type(task_type: str | None) -> str:
    task = str(task_type or "auto").strip().lower()
    ...
    if task in {"obb", "obb_detection", "oriented_detection", "rotated_detection"}:
        return "obb"
    if task in {"det", "detect", "detection", "object_detection", "yolo_detection"}:
        return "detection"
    return "auto"
...
def load_annotation_task_type(batch_dir: Path) -> str:
    for p in [batch_dir / "annotation_task.json", batch_dir / "annotation_classes.json"]:
        data = read_json(p, default={}) or {}
        task = data.get("task_type") or data.get("task")
        norm = normalize_task_type(str(task or ""))
        if norm != "auto":
            return norm
    return "auto"
...
def infer_label_format(label_paths: list[Path]) -> str:
    ...
    YOLO Seg: class x1 y1 ... xn yn -> >=7列且坐标成对。若超过 9 列，一定认为是 segmentation。

    注意：4 点 polygon 与 OBB 都是 9 列，无法仅靠 txt 严格区分。
    所以优先由 --task-type 或 annotation_task.json 指定。
    """
    max_cols = 0
    has_seg_like = False
    has_obb_like = False
...
def ensure_raw_dataset_dirs(output_root: Path) -> None:
    for sub in [
        "images/train",
        "images/val",
        "labels/train",
...
def write_data_yaml(output_root: Path, class_names: list[str], label_format: str) -> None:
    names_lines = "\n".join([f"  {i}: {name}" for i, name in enumerate(class_names)])
    task_line = "task: segment\n" if label_format == "segmentation" else ""
...
def output_root_for_format(label_format: str, default_output_root: str) -> Path:
    if label_format == "obb":
        return PROJECT_ROOT / "data" / "raw_obb"
    if label_format == "segmentation":
        return PROJECT_ROOT / "data" / "raw_segmentation"
    return PROJECT_ROOT / default_output_root


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Accept reviewed labels into raw_* dataset.")
    parser.add_argument(
        "--model-context",
        type=str,
    ...
    parser.add_argument(
        ...
        help="检测任务输出数据集目录。分割/OBB 会自动切到 data/raw_segmentation / data/raw_obb。",
    )
    parser.add_argument(
        "--task-type",
        type=str,
        default="auto",
        choices=["auto", "detection", "obb", "segmentation", "segment", "seg"],
        help="显式指定入库任务类型。auto 时从 annotation_task.json 或标签格式推断。",
    )
    parser.add_argument(
        "--val-ratio",
        type=float,
...
def main() -> int:
    ...
    if not labels_dir.exists():
        raise FileNotFoundError(
            f"未找到 labels 目录: {labels_dir}。请先在标注器中保存 YOLO 格式标注到该目录。"
        )
    ...
    if not pairs:
        raise RuntimeError(
            f"未找到可接收的图片/标签。请确认标注器已在 {all_images_dir} 保存同名 txt。"
        )

    explicit_task = normalize_task_type(args.task_type)
    saved_task = load_annotation_task_type(batch_dir)
    if explicit_task != "auto":
        label_format = explicit_task
        source = "--task-type"
    elif saved_task != "auto":
        label_format = saved_task
        source = "annotation_task.json"
    else:
        label_format = infer_label_format([label for _, label in pairs])
        source = "label format"

    output_root = output_root_for_format(label_format, args.output_root)
    if label_format == "obb":
        print(f"[INFO] 检测到 OBB 标注格式，来源={source}，输出到 data/raw_obb")
    elif label_format == "segmentation":
        print(f"[INFO] 检测到 segmentation 标注格式，来源={source}，输出到 data/raw_segmentation")
    else:
        print(f"[INFO] 检测到普通 detection 标注格式，来源={source}，输出到 data/raw_detection")

    print(f"[INFO] 清空输出目录: {output_root.relative_to(PROJECT_ROOT)}")
    reset_output_root(output_root)
    ensure_raw_dataset_dirs(output_root)
    ...
    write_data_yaml(output_root, class_names, label_format=label_format)
