  ...
  <div class="layout">
    ...
    <div class="right">
      ...
      <div class="hint">
        ...
        编辑：鼠标左键点击框选中；拖动框内部可整体移动；拖动矩形框边缘/角点可调整大小；拖动 OBB 顶点可微调角点；选中后按 Q 删除。<br>
...
let dirty = false;
let editMode = null;          // null | 'move' | 'resize' | 'obb-point'
let editHandle = null;        // bbox: tl/tr/bl/br/l/r/t/b；obb: 顶点下标
let editStartPt = null;       // 鼠标按下时的图像坐标
let editStartAnn = null;      // 鼠标按下时标注的深拷贝
...
function normBbox(ann) {
  return {
    x1: Math.min(Number(ann.x1 || 0), Number(ann.x2 || 0)),
    y1: Math.min(Number(ann.y1 || 0), Number(ann.y2 || 0)),
    x2: Math.max(Number(ann.x1 || 0), Number(ann.x2 || 0)),
    y2: Math.max(Number(ann.y1 || 0), Number(ann.y2 || 0)),
  };
}

function pointInBbox(p, ann) {
  const b = normBbox(ann);
  return p.x >= b.x1 && p.x <= b.x2 && p.y >= b.y1 && p.y <= b.y2;
}

function hitBboxHandle(p, ann) {
  const b = normBbox(ann);
  const tol = Math.max(7 / Math.max(scale, 1e-6), 4);
  const near = (a, b) => Math.abs(a - b) <= tol;
  const left = near(p.x, b.x1);
  const right = near(p.x, b.x2);
  const top = near(p.y, b.y1);
  const bottom = near(p.y, b.y2);

  if (left && top) return 'tl';
  if (right && top) return 'tr';
  if (left && bottom) return 'bl';
  if (right && bottom) return 'br';
  if (left && p.y >= b.y1 && p.y <= b.y2) return 'l';
  if (right && p.y >= b.y1 && p.y <= b.y2) return 'r';
  if (top && p.x >= b.x1 && p.x <= b.x2) return 't';
  if (bottom && p.x >= b.x1 && p.x <= b.x2) return 'b';
  return null;
}
...
function hitObbPoint(p, ann) {
  const pts = ann.points || [];
  const tol = Math.max(8 / Math.max(scale, 1e-6), 4);
  for (let i = 0; i < pts.length; i++) {
    const dx = p.x - pts[i][0];
    const dy = p.y - pts[i][1];
...
function hitTestAnn(p) {
  // 从后往前命中，重叠目标优先选中最后绘制/最后导入的框。
  for (let i = annotations.length - 1; i >= 0; i--) {
    const ann = annotations[i];
    if (ann.type === 'bbox') {
      const handle = hitBboxHandle(p, ann);
      if (handle) return {index: i, mode: 'resize', handle};
      if (pointInBbox(p, ann)) return {index: i, mode: 'move', handle: null};
    } else if (ann.type === 'obb') {
      const pointIndex = hitObbPoint(p, ann);
      if (pointIndex !== null) return {index: i, mode: 'obb-point', handle: pointIndex};
      if (pointInPolygon(p, ann.points || [])) return {index: i, mode: 'move', handle: null};
    }
  }
  return null;
}
...
function cursorForHit(hit) {
  ...
  if (hit.mode === 'obb-point') return 'crosshair';
  const h = hit.handle || '';
...
function resizeBboxAnn(ann, base, p, handle) {
  const b = normBbox(base);
  let x1 = b.x1, y1 = b.y1, x2 = b.x2, y2 = b.y2;
  if ((handle || '').includes('l')) x1 = clamp(p.x, 0, img.naturalWidth);
  if ((handle || '').includes('r')) x2 = clamp(p.x, 0, img.naturalWidth);
  if ((handle || '').includes('t')) y1 = clamp(p.y, 0, img.naturalHeight);
  if ((handle || '').includes('b')) y2 = clamp(p.y, 0, img.naturalHeight);
...
function drawEditHandles(ann) {
  ...
  if (ann.type === 'bbox') {
    const b = normBbox(ann);
    const handles = [
      [b.x1, b.y1], [(b.x1 + b.x2) / 2, b.y1], [b.x2, b.y1],
      [b.x1, (b.y1 + b.y2) / 2], [b.x2, (b.y1 + b.y2) / 2],
      [b.x1, b.y2], [(b.x1 + b.x2) / 2, b.y2], [b.x2, b.y2],
    ];
    handles.forEach(([x, y]) => {
      ctx.beginPath();
      ctx.rect(sx(x) - 4, sy(y) - 4, 8, 8);
      ctx.fill();
      ctx.stroke();
    });
  } else if (ann.type === 'obb') {
...
function startDraw(){
  resetEditState();
  selectedIndex=-1;
  drawingMode=true; dragging=false; startPt=null; tempPt=null; obbPoints=[]; pendingAnnotation=null;
  canvas.classList.add('draw-ready'); canvas.style.cursor='crosshair';
  document.getElementById('saveStatus').textContent='标注中';
  renderBoxList(); draw();
}
function stopDraw(){
  drawingMode=false; dragging=false; startPt=null; tempPt=null; obbPoints=[];
...
canvas.addEventListener('mousemove', e => {
  ...
  if (editMode && selectedIndex >= 0 && editStartPt && editStartAnn) {
    ...
    if (editMode === 'move') {
      if (ann.type === 'bbox') {
        ann.x1 = clamp(editStartAnn.x1 + dx, 0, img.naturalWidth);
        ann.x2 = clamp(editStartAnn.x2 + dx, 0, img.naturalWidth);
        ann.y1 = clamp(editStartAnn.y1 + dy, 0, img.naturalHeight);
        ann.y2 = clamp(editStartAnn.y2 + dy, 0, img.naturalHeight);
      } else if (ann.type === 'obb') {
        ann.points = editStartAnn.points.map(pt => [
    ...
    if (editMode === 'resize' && ann.type === 'bbox') {
      resizeBboxAnn(ann, editStartAnn, p, editHandle);
      markDirty(); renderBoxList(); draw(); return;
    }

    if (editMode === 'obb-point' && ann.type === 'obb') {
...
function handleCanvasMouseUp(e) {
  ...
  if (editMode) {
    resetEditState();
    document.getElementById('saveStatus').textContent = '已修改，需保存';
    draw();
    return;
  }
...
function deleteSelected(){
  ...
  if(selectedIndex < 0){ alert('请先用鼠标左键点击图中的目标框，或在右侧“当前标注”列表中选择要删除的框。'); return; }
