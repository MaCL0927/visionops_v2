...
def infer_rknn_report_path(paths_cfg: dict) -> Path:
    # segmentation_mlops.yaml 目前未显式给出 rknn_report，
    # 这里按 dvc.yaml 的 segmentation RKNN 输出约定推断：
    # models/export_segmentation/rknn_perf_report.json
...
def ensure_registration_run(
    ...
    with mlflow.start_run(run_name=f"register-{model_name}") as run:
        ...
        log_params_if_any({
            "task": registry_cfg.get("task", "segmentation"),
