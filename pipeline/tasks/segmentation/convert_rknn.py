...
def maybe_reexec_in_rknn_env(cfg: dict) -> None:
    ...
    if os.environ.get("VISIONOPS_RKNN_INTERNAL") == "1" or current_python == python_path:
        return
    ...
    env["PYTHONPATH"] = str(project_root) if not old_pythonpath else str(project_root) + os.pathsep + old_pythonpath

    cmd = [str(python_path), "-m", "pipeline.tasks.segmentation.convert_rknn", *sys.argv[1:]]
    ...
    result = subprocess.run(cmd, check=False, cwd=str(project_root), env=env)
...
def simulate_conversion(cfg: dict) -> dict:
    """
    未安装 rknn-toolkit2 时，生成占位结果，便于先打通 segmentation 分支。
    """
    print("警告: rknn-toolkit2 未安装，执行模拟 RKNN 转换（segmentation）")
    ...
    with rknn_path.open("wb") as f:
        f.write(b"RKNN_Segmentation_SIMULATED_MODEL")
...
def convert_to_rknn(cfg: dict) -> dict:
    ...
    if do_quantization:
        ...
        calibration_images = load_calibration_dataset(
            dataset_path=quant_cfg.get("dataset", "data/processed_segmentation/images/val"),
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"Segmentation RKNN 模型转换 | 目标平台: {cfg['target_platform']}")
    print("=" * 60)

    if not os.path.exists(onnx_path):
        print(f"错误: ONNX 模型不存在: {onnx_path}")
        print("请先运行 segmentation ONNX 导出阶段")
        sys.exit(1)
