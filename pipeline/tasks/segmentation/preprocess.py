...
def validate_label_file(
    ...
    for line_no, line in enumerate(text.splitlines(), start=1):
        ...
        if len(parts) < 7:
            raise ValueError(
                f"{label_path} 第 {line_no} 行格式错误，分割标签至少需要 7 列: cls x1 y1 x2 y2 x3 y3 ..."
            )

        if (len(parts) - 1) % 2 != 0:
            raise ValueError(
                f"{label_path} 第 {line_no} 行格式错误，坐标必须是 x,y 成对出现: {line}"
            )
...
def main() -> None:
    ...
    if not raw_root.exists():
        raise FileNotFoundError(f"原始分割数据根目录不存在: {raw_root}")
